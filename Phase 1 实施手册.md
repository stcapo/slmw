...
### 15.26 2026-05-28 12V 电源与 MOSFET 暖白通道复核记录

```text
测试目标：
  复核 12V 电源、P24 空载电平、原暖白 MOSFET 通道是否真正异常
用户反馈：
  测 12V 电源本身：
    黑表笔接 DC-，红表笔接 DC+
    cw 100 / ww 100 状态下均稳定约 12.02V
  测 P24 空载：
    断开 P24 -> MOSFET IN 后，串口发 ww 100
    P24 -> GND 在板上约 3.0V
  手动测原暖白 MOSFET 通道：
    断开 P24/P25 到原暖白 IN 的控制线
    使用 T5 3V3 短接到原暖白 MOSFET IN，GND 共地
    暖白亮了
当前判断：
  12V 电源稳定，供电问题基本排除
  原暖白 MOSFET 通道和黄线/暖白灯带回路可工作，因为板上 3V3 直驱 IN 能点亮暖白
  P24 空载只有约 3.0V，低于理想 3.3V，且接入 MOSFET 后也约 2.98V，处在模块 3-5V 输入规格下沿
  当前最可疑点从“原暖白 MOSFET 通道坏”转为：
    1. P24 实际输出电平/驱动能力不足或不稳定
    2. P24 到 MOSFET IN 的细鳄鱼夹/杜邦线/排针接触电阻偏大
    3. P24 当前硬件 PWM 配置/引脚复用导致输出高电平不如板上 3V3 直驱可靠
重要结论：
  云端、App、DP、12V 电源、灯带暖白支路、原暖白 MOSFET 通道都基本排除
  当前核心问题是 P24 控制信号这一路到 MOSFET IN 的有效驱动
下一步排查：
  1. 用同一根线做对照：
     用刚才 3V3 能点亮暖白的那根线，改接 P24，再发 ww 100
     如果仍不亮，线材问题可排除，P24 输出更可疑
     如果亮了，之前 P24 到 IN 的线/夹子/接触有问题
  2. 换 T5 输出脚：
     将 WW_PIN 从 24 临时改成 25 或其他支持 PWM 的脚，验证同一个 MOSFET 暖白通道
     若换脚后暖白能亮，说明 P24 不适合继续作为 WW 输出
  3. 临时工程方案：
     直接把 WW/CW 对调，把稳定可用的 P25 分配给暖白，P24 分配给冷白
     如果 P24 驱动冷白也弱，则说明 P24 整体不适合本模块输入
  4. 正式产品方案：
     增加一级电平转换/栅极驱动/三极管驱动，将 T5 3.0V GPIO 转成稳定 5V 控制信号
     或更换明确支持 3.3V 逻辑输入且输入电流更小的 MOSFET 驱动模块
```

### 15.27 2026-05-28 PWM 放大链路终局方案记录

```text
测试目标：
  在重复排查仍出现同样问题后，确定不再继续依赖 T5 GPIO 直推当前 MOSFET 模块
用户反馈：
  更换/复测后仍是同样问题
当前结论：
  T5 GPIO 直连当前 MOSFET 模块输入的方案不再作为可靠 Phase 1 方案
  根因方向：
    T5 P24/P25 输出约 3.0V，处于 MOSFET 模块 3-5V 输入规格下沿
    当前模块输入可能需要一定驱动电流，GPIO 直推余量不足
    指示灯亮不能证明负载通道已可靠满导通
终局解决方案：
  方案 A：加一级 3.3V -> 5V 驱动/缓冲级
    T5 P24/P25 只驱动中间级
    中间级输出稳定 5V PWM 给 MOSFET IN
    保留当前 MOSFET 模块和灯带接线
    优点：最贴近当前硬件，验证快
    缺点：需要新增小模块/器件，并注意是否反相
  方案 B：更换功率级
    换明确支持 3.3V GPIO 输入、输入电流很小、可 PWM 调光的 MOSFET/LED 调光模块
    优点：电路更干净，适合后续产品化思路
    缺点：需要新采购/等待
  方案 C：短期绕行验证
    暂时只用已验证稳定的通道验证 App/DP/PWM 逻辑
    不再把当前 MOSFET 模块作为最终功率级依据
当前推荐：
  Phase 1 若要尽快完成，应采用方案 A 或 B
  继续调当前直连链路意义不大
验收标准：
  串口 ww 100：暖白全亮
  串口 cw 100：冷白全亮
  串口 mix 50 50：两路同时亮
  逻辑分析仪：P24/P25 或驱动后 IN 端 duty 与串口一致
  万用表：T5 3V3 稳定，驱动级输出高电平稳定接近 5V 或功率级明确接受 3.3V
```

### 15.28 2026-05-28 3V3 直连能亮但 P24 PWM 不亮记录

```text
测试目标：
  区分“电压不够”与“P24 硬件 PWM 没有形成有效控制”的问题
用户反馈：
  将 MOSFET 暖白 IN 从 P24 PWM 改接到 T5 3V3 后，暖白瞬间点亮且非常亮
当前判断：
  用户判断“PWM 没放大”是正确方向
  更准确地说：MOSFET 功率通道能被稳定直流 3V3 打开，但当前 P24 硬件 PWM 输出没有被 MOSFET 当作有效满导通信号
  因此问题不是 12V、灯带、MOSFET 输出负载通道，而是 P24 PWM 输出形态/占空比/极性/驱动能力/接触中的某一项
关键差异：
  T5 3V3 是稳定直流高电平，相当于 100% ON
  P24 是由硬件 PWM 外设输出的信号，即使串口显示 100%，也必须用逻辑分析仪确认是否真的是 100% 高电平、是否反相、是否有毛刺、是否电平被拉低
固件修改：
  新增数字强制输出测试命令：
    p24hi：关闭硬件 PWM，将 P24 作为普通 GPIO 强制 HIGH
    p24lo：关闭硬件 PWM，将 P24 作为普通 GPIO 强制 LOW
    p25hi：关闭硬件 PWM，将 P25 作为普通 GPIO 强制 HIGH
    p25lo：关闭硬件 PWM，将 P25 作为普通 GPIO 强制 LOW
    pwm：恢复硬件 PWM 输出
  编译结果：BUILD SUCCESS
下一步判断：
  若 p24hi 能点亮暖白，而 ww 100 不能：
    根因在 T5 硬件 PWM 配置/极性/占空比更新，不是 P24 普通 GPIO 电压不够
    后续可以改用软件 PWM 或修正硬件 PWM polarity/duty
  若 p24hi 也不能点亮，但 T5 3V3 能点亮：
    根因在 P24 GPIO 输出电流/电平/接触，不适合直推该 MOSFET IN
    后续需要换脚或加驱动级
  若 p25hi 能点亮而 p24hi 不能：
    P24 针脚/复用/接触问题成立，换 P25 或其他 PWM 脚
  若 p24hi / p25hi 都能点亮：
    两个 GPIO 直流输出可用，硬件 PWM 外设路径是主要嫌疑
```

### 15.29 2026-05-28 逻辑分析仪测 P24/P25 PWM 方案

```text
测试目标：
  使用逻辑分析仪确认 P24/P25 的实际波形是否符合固件预期，停止依赖万用表 PWM 平均电压
用户决定：
  当前 P24/P25 互换后的物理接线状态不再改回
  后续测试和记录以当前接线为准
  逻辑分析仪记录时优先按引脚编号记录：CH0=P24，CH1=P25
接线方式：
  LA GND -> T5 GND / MOSFET GND / 12V 电源负极共地点
  LA CH0 -> T5 P24
  LA CH1 -> T5 P25
  先只测 T5 引脚，不接 LA 到 12V+、灯带红线、MOSFET OUT 端
  若后续要测 MOSFET IN，CH0/CH1 可移到 MOSFET IN 端，但仍然必须以 GND 为参考
安全边界：
  逻辑分析仪只测 3.3V 数字信号
  不要把 CH 探头接到 12V 正极或灯带输出大电压点
  不需要接 LA 的 VCC
软件设置：
  采样率：1MHz 起步，能清楚看 200Hz PWM 即可
  采样时间：1s-5s
  阈值：若软件可选，选 3.3V CMOS 或阈值约 1.5V
  通道：启用 CH0/CH1
  触发：可不设触发，直接采集；若要触发，设 CH0 上升沿
串口测试命令：
  p24hi：P24 普通 GPIO 常高
  p24lo：P24 普通 GPIO 常低
  p25hi：P25 普通 GPIO 常高
  p25lo：P25 普通 GPIO 常低
  pwm：恢复硬件 PWM
  ww 100：固件认为 P24=100%、P25=0%
  cw 100：固件认为 P24=0%、P25=100%
  mix 50 50：固件认为 P24=50%、P25=50%
  mix 10 90：固件认为 P24=10%、P25=90%
  mix 90 10：固件认为 P24=90%、P25=10%
期望波形：
  频率约 200Hz，周期约 5ms
  10% duty：高电平约 0.5ms，低电平约 4.5ms
  50% duty：高电平约 2.5ms，低电平约 2.5ms
  90% duty：高电平约 4.5ms，低电平约 0.5ms
  100% duty：应接近常高
  0% duty：应接近常低
判断规则：
  p24hi 常高但 ww 100 不是常高：
    P24 普通 GPIO 可用，硬件 PWM 配置/极性/占空比路径异常
  p24hi 也不是常高：
    P24 引脚/接触/复用问题
  p25hi 正常但 p24hi 异常：
    P24 单脚问题优先
  mix 50 50 显示两路都是 200Hz 50%：
    PWM 输出基本符合预期，后续查 MOSFET 输入/功率级
  波形反相：
    若命令 100% 显示常低、0% 显示常高，则需要调整 PWM polarity 或 duty 反相
记录表：
  命令：
  CH0/P24 波形：
  CH1/P25 波形：
